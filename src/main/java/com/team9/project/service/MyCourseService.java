...
public class MyCourseService {
    ...
    public List<Map<String, String>> getCourseMapList(String para, @Nullable String para2, int mode) {
        ...
        Course c;
        Map<String, String> m;
        String courseNameParas, teacherParas, withdrawParas;
        for (Course course : cList) {
            c = course;
            m = new HashMap<>();
            m.put("id", String.valueOf(c.getId()));
            courseNameParas = "model=course&courseInfo=" + c.getCourseId();
            ...
            withdrawParas = "model=message&message=已退选&command=4&caller=myCourse&id=" + c.getId() + "&id2=" + studentMapper.getStudentByStudentInfo(para).get(0).getId() + "&callerStatus1=studentId&callerStatus2=" + para + "&callerStatus3=courseInfo&callerStatus4=" + para2;
    ...
    public DataResponse myCourseInit(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if (studentId == null) {
            ...
            dataList = getCourseMapList(studentId, null, 2);
    ...
    public DataResponse myCourseQuery(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if ("".equals(paraKeeper)) {
            dataList = getCourseMapList(courseInfo, null, 1);
        } else {
            dataList = getCourseMapList(paraKeeper, courseInfo, 3);
        }
